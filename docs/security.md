...
- **シークレット管理**: Bitwarden/1Password CLI 経由でのみアクセス
...
Git の user.name / user.email やテンプレート変数は、Bitwarden CLI 経由で取得します。`make bw-unlock` で vault をアンロックし、環境変数を設定してから `chezmoi apply` を実行します。
