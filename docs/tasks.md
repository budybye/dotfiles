---
name: tasks
description: タスク管理 進捗整理
---
# タスク管理 - Chezmoi Dotfiles Management System

## 概要

このドキュメントは、プロジェクトのタスク管理と実行計画を記載します。AI エージェントによる自動管理と更新を想定しています。

## タスク管理方法

- **AI 管理**: タスクの自動追跡と進捗管理
- **定期的なレビュー**: 週次または月次の進捗確認
- **優先度ベース**: 重要度に応じた実装順序

## タスクカテゴリ

### 開発タスク

#### 新機能の追加

- [ ] 機能要件の明確化
- [ ] 設計レビュー
- [ ] 実装
- [ ] テスト
- [ ] ドキュメント更新

#### 既存機能の改善

- [ ] 問題点の特定
- [ ] 改善案の検討
- [ ] 実装
- [ ] テスト
- [ ] ドキュメント更新

#### バグ修正

- [ ] バグの再現確認
- [ ] 原因調査
- [ ] 修正実装
- [ ] テスト
- [ ] ドキュメント更新

### メンテナンスタスク

#### ドキュメント更新

- [ ] ドキュメントの整合性確認
- [ ] 古い情報の更新
- [ ] 新機能の反映
- [ ] 参照リンクの確認

#### 依存関係の更新

- [ ] パッケージバージョンの確認
- [ ] セキュリティ更新の適用
- [ ] 互換性テスト
- [ ] ドキュメント更新

#### セキュリティパッチ適用

- [ ] セキュリティ情報の確認
- [ ] 影響範囲の調査
- [ ] パッチの適用
- [ ] テスト
- [ ] ドキュメント更新

### テストタスク

#### クロスプラットフォーム動作確認

- [ ] macOS での動作確認
- [ ] Ubuntu での動作確認
- [ ] Docker 環境での動作確認
- [ ] 問題の修正

#### CI/CD パイプラインの検証

- [ ] ワークフローの実行確認
- [ ] テスト結果の確認
- [ ] エラーの調査と修正
- [ ] パフォーマンスの最適化

## タスクテンプレート

### 新機能追加テンプレート

```markdown
## [機能名]

### 要件

- [ ] 要件の明確化
- [ ] 技術的制約の確認

### 設計

- [ ] 設計書の作成・更新
- [ ] レビュー

### 実装

- [ ] 実装
- [ ] コードレビュー

### テスト

- [ ] 単体テスト
- [ ] 統合テスト
- [ ] クロスプラットフォームテスト

### ドキュメント

- [ ] ドキュメント更新
- [ ] 参考資料の追加
```

### バグ修正テンプレート

```markdown
## [バグ名/番号]

### 調査

- [ ] 再現手順の確認
- [ ] 原因の特定
- [ ] 影響範囲の調査

### 修正

- [ ] 修正実装
- [ ] テスト

### 確認

- [ ] クロスプラットフォーム動作確認
- [ ] ドキュメント更新
```

## 優先度定義

1. **緊急**: セキュリティ問題、システム停止など
2. **高**: 重要な機能追加、重大なバグ修正
3. **中**: 機能改善、通常のバグ修正
4. **低**: 最適化、リファクタリング、ドキュメント改善

## 運用ルール

### タスク追加時

1. 計画が必要な規模（新 OS 対応、大規模変更）→ `.cursor/plans/YYYY-MM-DD-<feature>.md` を作成
2. 計画不要な小規模タスク → 本ドキュメントの「具体的バックログ」に直接追加
3. `.cursor/plans/` の既存計画一覧に新規計画を追加（計画索引が必要な場合は `docs/plan.md` を作成）

### タスク完了時

1. 該当計画があればステータスを「完了」に更新
2. `docs/requirements.md` の対応 OS ステータスを更新
3. `README.md` の「予定」「対応済み」を更新
4. 本ドキュメントのバックログから該当タスクを削除または完了マーク

## 具体的バックログ

### 高優先度

| タスク | 計画 | ステータス |
| ------ | ---- | ---------- |
| WSL2 CI ジョブ有効化 | [2025-02-19-wsl2-support](../.cursor/plans/2025-02-19-wsl2-support.md) | 未着手 |
| SANDBOX 環境変数プロファイル | [chezmoi sandbox plan](../.cursor/plans/chezmoi%20sandbox-7eb45bfb.plan.md) | 未着手 |

### 中優先度

| タスク | 計画 | ステータス |
| ------ | ---- | ---------- |
| bash-only CLI + 2-step minimal dotfiles | [2025-03-27-bash-and-clissh](../.cursor/plans/2025-03-27-plan-bash-varsion-and-clissh-varsion) | 未着手 |
| （将来）Windows ネイティブ対応 | 未作成 | 保留 |
| （将来）FreeBSD 対応 | 未作成 | 保留 |

### 低優先度

| タスク | 計画 | ステータス |
| ------ | ---- | ---------- |
| mise ツールのプロジェクト別最適化 | 未作成 | 検討中 |
| ドキュメント相互参照の自動チェック | 未作成 | アイデア |

## 進捗追跡

### ステータス

- **未着手**: タスクが作成されたが、まだ開始していない
- **進行中**: 現在作業中
- **レビュー中**: 実装完了、レビュー待ち
- **完了**: すべてのチェック項目が完了
- **保留**: 一時的に保留

## 実装計画との連携

新機能やバグ修正を実装する際は `.cursor/plans/` に詳細な実装計画を作成することを推奨します（注: 計画ファイルは git 管理下にあり、バージョン管理されます）。

- **計画作成**: `.cursor/plans/TEMPLATE.md` をコピーし、`YYYY-MM-DD-<feature>.md` 形式で保存
- **実行**: 各タスクを bite-sized（2-5分）で実施し、コミットを細かく分ける
- **検証**: `make check`、`chezmoi diff`、GitHub Actions で動作確認

### 計画ファイルの管理

計画ファイルは `.cursor/plans/` ディレクトリに保存し、以下のルールに従います：

- **ファイル形式**: `YYYY-MM-DD-<feature>.md` (日付接頭辞でソート可能)
- **テンプレート**: `.cursor/plans/TEMPLATE.md` をコピーして使用
- **参照**: 本ドキュメントのバックログから計画ファイルへのリンクを管理
- **バージョン管理**: 計画ファイルは git 管理下にあり、進捗に応じて更新される

## 関連ドキュメント

- [要件定義](./requirements.md)
- [設計書](./design.md)
- [技術スタック](./tech.md)
- [ディレクトリ構成](./directory.md)
