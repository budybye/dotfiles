...
AI Gateway 経由で **Unified Billing + ZDR** で OpenAI / Anthropic / Google を使用する Pi 拡張です。

- **`cf-ai-gateway`** – `provider/model-name` 形式のモデルIDで各プロバイダーにリクエスト
...
1. `~/.pi/agent/auth.json` から `cloudflare-ai` ブロック（`access` / `account_id`）を削除
2. 旧 `bneil` 拡張をアンインストール（`~/.pi/agent/extensions/` 配下削除または npm パッケージ削除）
3. `/cf-ai-setup` で `cf-ai-gateway` を設定し直す
