...
- ライブラリやSDKの**依存関係はなるべく減らし**て、可読性が下がる程の長いコードはなるべく分割して、適切なディレクトリ構造で、管理しやすくしてください。
- フォーマット、リントなどは適切なタイミングで git commit を分ける様にして余計な差分を増やさない様にしてください。
- **命名やコメントは短くてわかりやすい**かを真剣に考えて、コードの可読性と保守性を高める様にしてください。
- **Web標準、RFC、W3C勧告、ISOなどの技術標準**の一次情報を参照したり積極的に採用して、汎用性と互換性を高める様に設計、開発してください。
